#!/bin/bash

set -e

RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

log_info() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

log_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1"
}

log_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

log_error() {
    echo -e "${RED}[ERROR]${NC} $1"
}

main() {
    log_info "Checking nginx processes and services..."
    
    log_info "Checking nginx processes..."
    if pgrep nginx >/dev/null 2>&1; then
        log_error "Nginx processes found:"
        ps aux | grep nginx
        echo ""
        log_info "Killing nginx processes..."
        sudo pkill -9 nginx || true
        sudo killall -9 nginx || true
        sleep 2
        if pgrep nginx >/dev/null 2>&1; then
            log_error "Nginx processes still running after kill!"
            ps aux | grep nginx
        else
            log_success "Nginx processes killed"
        fi
    else
        log_success "No nginx processes running"
    fi
    
    log_info "Checking nginx service..."
    if sudo systemctl is-active nginx >/dev/null 2>&1; then
        log_error "Nginx service is active:"
        sudo systemctl status nginx --no-pager
        echo ""
        log_info "Stopping nginx service..."
        sudo systemctl stop nginx || true
        sudo systemctl disable nginx || true
        if sudo systemctl is-active nginx >/dev/null 2>&1; then
            log_error "Nginx service still active after stop!"
        else
            log_success "Nginx service stopped"
        fi
    else
        log_success "Nginx service is not active"
    fi
    
    log_info "Checking nginx command..."
    if command -v nginx >/dev/null 2>&1; then
        log_warning "Nginx command exists:"
        which nginx
        nginx -v
    else
        log_success "Nginx command not found"
    fi
    
    log_info "Checking nginx directories..."
    if [[ -d "/etc/nginx" ]]; then
        log_warning "Nginx config directory exists:"
        sudo ls -la /etc/nginx/
    else
        log_success "Nginx config directory not found"
    fi
    
    log_info "Checking port bindings..."
    echo "Port 80:"
    sudo netstat -tlnp | grep :80 || echo "Port 80 is free"
    
    echo "Port 443:"
    sudo netstat -tlnp | grep :443 || echo "Port 443 is free"
    
    log_info "Checking nginx packages..."
    if dpkg -l | grep nginx; then
        log_warning "Nginx packages installed:"
        dpkg -l | grep nginx
    else
        log_success "No nginx packages installed"
    fi
    
    log_info "Checking systemd nginx files..."
    if [[ -f "/lib/systemd/system/nginx.service" ]]; then
        log_warning "Nginx systemd service file exists:"
        sudo ls -la /lib/systemd/system/nginx.service
    else
        log_success "Nginx systemd service file not found"
    fi
    
    log_success "Nginx processes check completed!"
}

main "$@"
